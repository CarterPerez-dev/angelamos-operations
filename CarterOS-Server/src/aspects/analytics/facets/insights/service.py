...
class InsightsService:
    ...
    async def get_performance_rankings(
        ...
        def to_video_performance(video) -> VideoPerformance:
            return VideoPerformance(
                ...
                hook=video.hook,
                views=video.views,
                engagement_rate=InsightsRepository.calculate_engagement_rate(video),
                follower_conversion_rate=InsightsRepository.calculate_follower_conversion_rate(video),
                avg_watch_time=video.avg_watch_time,
                watched_full_video_percentage=video.watched_full_video_percentage,
                date_posted=video.date_posted.isoformat(),
            )

        by_views = sorted(videos, key=lambda v: v.views, reverse=True)[:limit]
        by_engagement = sorted(
            videos,
            key=lambda v: InsightsRepository.calculate_engagement_rate(v),
            reverse=True
        )[:limit]
        by_followers = sorted(
            videos,
            key=lambda v: InsightsRepository.calculate_follower_conversion_rate(v),
            reverse=True
        )[:limit]
        by_watch_time = sorted(
            videos, key=lambda v: v.avg_watch_time, reverse=True
        )[:limit]

        return PerformanceRankingsResponse(
            by_views=[to_video_performance(v) for v in by_views],
            by_engagement_rate=[to_video_performance(v) for v in by_engagement],
            by_follower_conversion=[to_video_performance(v) for v in by_followers],
            by_watch_time=[to_video_performance(v) for v in by_watch_time],
        )
    ...
    async def get_traffic_source_insights(
        ...
        source_data = await InsightsRepository.aggregate_traffic_sources(session)
    ...
    async def get_search_query_insights(
        ...
        query_data = await InsightsRepository.aggregate_search_queries(session)
    ...
    async def get_comment_word_insights(
        ...
        word_data = await InsightsRepository.aggregate_comment_words(session)
    ...
    async def get_video_length_insights(
        ...
        length_data = await InsightsRepository.aggregate_by_length_range(session)
    ...
    async def get_hashtag_insights(
        ...
        hashtag_data = await InsightsRepository.aggregate_hashtag_performance(session)
    ...
    async def get_posting_time_insights(
        ...
        day_data = await InsightsRepository.aggregate_by_posting_time(session)
